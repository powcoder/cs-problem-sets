...
public class BST<T extends Comparable<T>> {
	...
	public ArrayList<T> sort() {
		...
		inorder(root, list); //root is the binary search tree, list will hold all the keys in ascending order
		return list;
	}

	/**
	 * The parameter list is used to hold all the keys, and it has to be passed on as a parameter 
	 * when we make recursive calls at lines 139 and 143 to ensure all keys will be stored in the same list.
	 * 
	 * @param root
	 * @param list, used to hold all the keys
	 */
	...
	public static void main(String[] args) {		
		...
		// sort
		//first build a binary search tree named as strBST
		BST<String> strBST = new BST<String>();
		String[] arr = {"Lucy", "Hall", "Stone", "Till", "Hill", "Core"};
		for (String item: arr) {
			strBST.insert(item);
		}
		//call the sort method which performs the inorder traversal
		ArrayList<String> res = strBST.sort();
		System.out.println("\nSorted: " + res);
